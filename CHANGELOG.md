...
- Chat interface (#28): `/chat` renders grounded answers with `[SOURCE N]` markers as numbered badges (`AnswerCard`), structured citation cards with null-safe metadata (`CitationList`), and the mandatory legal disclaimer on every answer (`LegalDisclaimer`). `QueryInput` composer submits on Enter and offers a union filter (populated from `GET /documents`) that injects the canonical union name into the query text, since `POST /query` derives union filters from the text itself. Distinct error states for 429 (rate/tier limit) and 503 (upstream unavailable); loading indicator while a query is in flight. No streaming in Phase 3. README refreshed to the current Phase 3 state.
...
- Production web images are now built with `NEXT_PUBLIC_API_URL` baked in, sourced from the `PROD_API_URL` repository variable. Dokploy pulls the CI-built image (it does not build from the Dockerfile), so the API origin has to be passed at build time in `deploy-prod.yml`; the build job fails fast if `PROD_API_URL` is unset rather than shipping a frontend that calls the wrong origin.
